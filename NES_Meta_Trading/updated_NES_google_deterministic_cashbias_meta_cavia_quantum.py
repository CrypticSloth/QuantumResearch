...
def predict(inputs, weights, context_params, bs=False, num_wires=num_wires):
    '''
    ...
    # print("Inputs: ", inputs)
    # print("CP : ", context_params)
    inputs = np.concatenate((inputs, np.array(context_params).reshape([2,1])))
    # print("Inputs Conc: ", inputs)
...
class Agent:
    ...
    def __init__(
        self, money, limit, close, window_size, skip, num_portfolios, num_stocks, num_days, split, context_params, theta, bs
    ):
    ...
    def act(self, sequence):
        decision = predict(np.array(sequence).reshape(self.num_stocks,self.window_size), self.theta, self.context_params, bs=self.bs, num_wires=self.num_stocks+1) # num_wires = num_stocks + 1 becuase we need to add the cash bias layer
        # print(decision)
